...
navCategory.addEventListener('click', () => {
    ...
    if (str.includes('menu_white')) {
        menuIcon.src = '/BABOOK_Project/img/cross_white.png';
        // 메뉴 보이기
        fullMenu.classList.remove('off');
    }
    else {
        menuIcon.src = '/BABOOK_Project/img/menu_white.png';
        // 메뉴 감추기
...
/* 로그인 & 회원가입 페이지가 아닌 경우에만 동작(수정 필요) */
//console.log(location.href);
if((location.href.indexOf("login") && location.href.indexOf("signUp")) == -1){
    navigo();
}