...
void show_member_point(char id[])
{
	...
	while (OnLoop_A == true)
	{
		...
		{
		case 1:
			printf("1. 포인트 충전을 시작합니다.\n");
			show_point_charge(id);
			OnLoop_A = false;
			break;
		case 2:
			printf("2. 포인트 현금반환을 시작합니다.\n");
			OnLoop_A = false;
			break;
		case 3:
			printf("3. 이전 메뉴로 돌아갑니다.\n");
			OnLoop_A = false;
			break;
		default:
			printf("잘못된 선택입니다. 위의 메뉴 중 하나를 선택해주세요: ");
