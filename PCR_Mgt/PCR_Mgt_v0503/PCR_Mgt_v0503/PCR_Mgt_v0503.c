...
void show_member_time(char id[]); // 회원 | 3. PC방 이용시간 관리 메뉴
...
void show_member_menu(char id[])
{
	...
	while (OnLoop_A == true)
	{
		...
		case 3:
			...
			break;
		case 4:
			printf("4. PC방 이용하기를 선택하셨습니다.\n");
			OnLoop_A = false;
			break;
		case 5:
			printf("5. 종료하기를 선택하셨습니다.\n");
			OnLoop_A = false;
			break;
		default:
...
void show_point_charge(char id[])
{
	...
	while (!feof(fp))
	{
		for (i = 0; i < fmember_count; i++)
		{
			...
			{
				temp = i;
				//printf("2현재 %s 회원님의 포인트 잔액: %d원입니다.\n", fm[i].name, fm[i].mpoint);
			}
			else
			{
				//break;
...
void show_point_refund(char id[])
{
	...
	while (!feof(fp))
	{
		for (i = 0; i < fmember_count; i++)
		{
			...
			{
				temp = i;
				//printf("3현재 %s 회원님의 포인트 잔액: %d원입니다.\n", fm[i].name, fm[i].mpoint);
			}
			else
			{
				//break;
...
{
	int charge_time; // 충전시간

	int menu_choice; // 순번 선택 저장
	int i; // 반복 변수
	int temp; // 회원 정보 변수
	bool OnLoop_A = true; // while & switch 반복문 조절
