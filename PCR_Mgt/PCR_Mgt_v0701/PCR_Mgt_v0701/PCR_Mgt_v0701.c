...
void main_page();                   // 메인 페이지 | 회원/비회원 선택 메뉴
void show_n_member_menu();          // 비회원 메뉴 
void show_member_register();        // 회원가입 메뉴
void show_n_member_pay();           // 비회원 이용시간/요금 결제 메뉴 
void show_seat(int w_time);         // 좌석 선택 메뉴 
...
void show_password_edit(char id[]); // 회원 | 1.1 비밀번호 수정 메뉴
void show_withdraw(char id[]);      // 회원 | 1.2 회원탈퇴 메뉴
...
void show_member_list_edit();     // 관리자 | 3. 회원 정보수정 메뉴
void show_seat_mgt();             // 관리자 | 4. 좌석 관리 메뉴
...
{
	struct member m = { 0, }; // 회원 정보 저장 변수 선언
	struct fmember fm[fmember_count] = { 0, }; 	//파일 내 회원 정보 변수 선언
	int i; // 반복문 변수
...
void show_login()
{
	...
	if (strcmp(m.id, admin_id) == 0) // 관리자인 경우
	{
		...
		{
			printf("관리자 비밀번호가 잘못되었습니다. 프로그램을 다시 실행해주세요. \n");
		}
	}
...
// 회원 | 1.1 비밀번호 수정 메뉴
void show_password_edit(char id[])
{
	int i; // 반복 변수
	int temp; // 임시 변수
	char sel; // 메뉴 선택 변수
	struct member m = { 0, }; // 멤버 구조체
	struct fmember fm[fmember_count] = { 0, }; // 파일 멤버 구조체
	FILE* fp;
	...
	printf("수정하실 비밀번호를 입력해주세요(4자):");
	scanf(" %s", &m.password);

	// 수정 비밀번호 저장
	strcpy(fm[temp].password, m.password);
...
// 회원 | 1.2 회원 탈퇴 메뉴
void show_withdraw(char id[])
{
	char sel; // 메뉴 선택 변수
	int i; // 반복변수
	int temp; // 임시변수
	struct member m = { 0, }; // 멤버 구조체 변수
	struct fmember fm[fmember_count] = { 0, }; // 파일 멤버 구조체 변수
	FILE* fp;

	printf("정말로 회원에서 탈퇴하시겠습니까? (y/n): ");
	scanf(" %c", &sel);
	...
	if (sel == 'y')
	{
		...
		if (fp == NULL)
		{
			fprintf(stderr, "회원 탈퇴를 할 수 없습니다.\n");
			exit(1);
		}
		...
		// 회원 정보를 초기화하기
		strcpy(fm[temp].name, m.name);
		strcpy(fm[temp].tel, m.tel);
		strcpy(fm[temp].id, m.id);
		strcpy(fm[temp].password, m.password);
		fm[temp].mpoint = 0;
		fm[temp].mtime = 0;
		...
		if (fp == NULL)
		{
			fprintf(stderr, "회원 정보를 초기화할 수 없습니다.\n");
			exit(1);
		}
...
void show_member_list()
{
	...
	while (!feof(fp))
	{
		...
		{
			fscanf(fp, "%s %s %s %s %d %d", fm[i].name, fm[i].tel, fm[i].id, fm[i].password, &fm[i].mpoint, &fm[i].mtime);
			printf("%3d  %s %s %s\n", i + 1, fm[i].name, fm[i].tel, fm[i].id);
		}
		break;
	}
	fclose(fp);
...
// 관리자 | 3. 회원 정보수정 메뉴
void show_member_list_edit()
{
	char sel; // 메뉴 선택 변수
	int member_num; // 수정할 회원 번호 변수
	int i; // 반복 변수
	int temp = 0; // 회원 정보 변수
	...
	printf("------------------------------------------\n");
	printf("수정하고 싶은 회원의 번호를 입력해주세요. (수정은 이름과 핸드폰 번호만 가능합니다.):");
	scanf("%d", &member_num);

	printf("수정하실 이름을 입력해주세요(3자): ");
	scanf(" %s", &fm[member_num - 1].name);

	printf("수정하실 핸드폰번호를 입력해주세요(ex. 000-0000-0000): ");
	scanf(" %s", &fm[member_num - 1].tel);
	...
	if (fp == NULL)
	{
		fprintf(stderr, "회원 목록을 열 수 없습니다.\n");
		exit(1);
	}
...
void show_seat_mgt()
{
	...
	int fseat[seats_size]; // 좌석 이용 현황
	int onseat = 1; // 좌석 이용시
	char reset1; // 좌석 초기화 여부
	char reset2; // 전체 좌석 초기화 여부
	char sel; // 메뉴 선택 변수
	int seat_choice; // 초기화를 원하는 좌석

	FILE* fp_seat;

	printf("==========================================\n");
	printf("4. 좌석 관리 메뉴입니다. 환영합니다.\n\n");
	printf("  현재 좌석 이용 현황\n");
	...
	printf("\n");
	printf("좌석 현황을 초기화 하시겠습니까? (y/n): ");
	scanf(" %c", &reset1);

	if (reset1 == 'y')
	{
		printf("전체 좌석을 초기화 하시겠습니까? (y/n): ");
		scanf(" %c", &reset2);

		if (reset2 == 'y')
		{
			printf("전체 좌석을 초기화합니다.\n");
			for (i = 0; i < seats_size; i++)
			{
				fseat[i] = 0;
			}
		...
		{
			printf("초기화를 원하시는 좌석을 입력해주세요: ");
			scanf("%d", &seat_choice);
			fseat[seat_choice - 1] = 0;
		}
		...
		if (fp_seat == NULL)
		{
			fprintf(stderr, "좌석 이용현황을 저장할 수 없습니다.\n");
			exit(1);
		}
