...
void GolfManager::SetMemVisit(int num)
{
	memVisit = num;
}
...
void GolfManager::Login()
{
	...
	while (getline(fin, line)) { // 파일에서 문자열 가져오기
		...
		string fmember[6];     // 파일 회원 정보 저장 배열
		for (int i = 0; i < 6; i++) {
		...
		if (userid.compare(fmember[2]) == 0) {
			if (userpw.compare(fmember[3]) == 0) {
				...
				SetMemType(1);             // 회원 타입 저장
				SetMemVisit(0);            // 방문 횟수 구분자 초기화
...
void GolfManager::Visit_GetPoint()
{
	// 1. 방문 횟수에 따른 포인트 지급
	// 1.1 회원 정보 가져오기
	ifstream fin;
	...
	string fmember2[MEMCOUNT][6]; // 파일 회원 정보 저장 배열
	int count = 0;               // 회원 수
	while (getline(fin, line)) { // 파일에서 문자열 가져오기
		// 파일에 저장된 데이터 가져오기
		stringstream ss(line); // 문자열 자르기
		for (int j = 0; j < 6; j++) {
			ss >> fmember2[count][j]; // 잘린 문자열 가져오기	
		}
		if (GetMemberID().compare(fmember2[count][2]) == 0) {
	...
	// 1.2 방문 횟수 & 포인트 업데이트
	int visit = stoi(fmember2[mIndex][5]); // 기존 회원 방문 횟수
	int sumVisit = visit + 1;              // 방문 횟수 1 증가
	fmember2[mIndex][5] = to_string(sumVisit);

	// 방문 횟수에 따른 포인트 지급
	int point = stoi(fmember2[mIndex][4]); // 기존 회원 포인트
	int sumPoint = 0; // 합산 포인트
	int addPoint = 0; // 방문 횟수에 따른 포인트 지급량
	if (sumVisit == 1) { // 첫 방문 시
		addPoint = 1000;
	}
	else if (sumVisit == 5) { // 5회째 방문시
		addPoint = 5000;
	}
	else if (sumVisit == 10) { // 10회째 방문시
		addPoint = 10000;
	}
	else { // 일반 방문시
		addPoint = 100;
	...
	sumPoint = point + addPoint; // 포인트 증가
	fmember2[mIndex][4] = to_string(sumPoint);
...
void GolfManager::Run_Golf()
{
	...
	if (GetMemberID().compare("none_ID") == 0) { // 비회원인 경우
		...
		int mIndex = 0; // 회원 정보 위치
		string fmember[MEMCOUNT][6]; // 파일 회원 정보 저장 배열
		int mcount = 0; // 회원 수
...
void GolfManager::Swing_Golf(int gameCount, int sltCourse, string courseName, int sltDistance)
{
	...
	for (int i = 0; i < gameCount; i++) {
		...
		case 3:
			...
			break;
		case 4:
			cout << "Contraturation! 버디(Birdie): -1" << endl;
			score = standardPar - 1;
			break;
		case 5:
...
void GolfManager::MemberMenu()
{
	// 1. 방문 포인트 지급
	if (GetMemVisit() == 0) {
		Visit_GetPoint(); // 포인트 & 방문 횟수 증가
		SetMemVisit(1);   // 방문 횟수 구분자 변경
	}
...
void GolfManager::Delete_Course()
{
	...
	// 2.2 골프 코스 번호 검사
	if (key > count) {
		cout << "잘못된 입력입니다. 코스 번호를 확인해주세요." << endl;
		Manage_Course();
	}
